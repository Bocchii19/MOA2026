...
//!
//! **Local validation can sweep `RUNS` independently-seeded configurations.** Arena
//! measurement must use `--single-run --seed-index N`, so one remote job performs exactly
//! one QKV launch, one SAO launch and one FFN launch.
...
fn run_mode() -> RunMode {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--seed-index" => {
                let value = args.next().unwrap_or_else(|| panic!("--seed-index requires an integer in 0..{RUNS}"));
                seed = Some(value.parse::<usize>().unwrap_or_else(|_| panic!("invalid --seed-index `{value}`; expected an integer in 0..{RUNS}")));
            }
            "--help" | "-h" => {
                println!("Usage: test_kernels [--single-run --seed-index N]");
                std::process::exit(0);
            }
    ...
    match (single, seed) {
        (false, None) => RunMode::Sweep,
        (true, Some(index)) if index < RUNS => RunMode::Single(index),
        (true, Some(index)) => panic!("--seed-index {index} is outside 0..{RUNS}"),
        (true, None) => panic!("--single-run requires --seed-index N"),
        (false, Some(_)) => panic!("--seed-index is only valid with --single-run"),
    }
}
...
async fn main() {
    ...
    let selected_runs: Vec<usize> = match mode {
        RunMode::Sweep => (0..RUNS).collect(),
        RunMode::Single(seed) => vec![seed],
    };
    match mode {
        RunMode::Sweep => println!(
            "NPU kernel tests -- {} cases x {RUNS} independently-seeded runs against a precomputed reference{}\n",
            TESTS.len(), if profile { ", with on-device cycle counts" } else { "" }
        ),
        RunMode::Single(seed) => println!(
            "measurement mode=single-run seed={seed} expected_launches={}{}\n",
            TESTS.len(), if profile { " profile=enabled" } else { " profile=disabled" }
        ),
    }
    ...
    for test in TESTS {
        ...
        let mut all_ok = true;
        let mut cycles_by_run = Vec::with_capacity(selected_runs.len());
        for &run in &selected_runs {
            if matches!(mode, RunMode::Single(_)) {
                println!("launch kernel={} seed={run}", test.name);
            }
            if profile {
                collector.clear();
            }
        ...
        if profile {
            ...
            } else if let RunMode::Single(seed) = mode {
                assert_eq!(cycles_by_run.len(), 1, "single-run mode recorded multiple cycle windows");
                println!("    cycles={} run={seed}", cycles_by_run[0]);
            } else {
                println!(
