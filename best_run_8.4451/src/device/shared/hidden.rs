...
fn root_mean_square<C: M>(
    ...
    // Preserve the complete-H mean, EPS placement and DM boundary. Only
    // bypass the scalar sqrt-result DM commit/reload into the consumer VRF.
    let mean_square: DmTensor<f32, Chip, C, HiddenChunks, m![1 # 8]> =
        unsafe { mean_square.reshape() };
    ctx
        .main
...
fn root_mean_square_fused<C: M>(
    ...
    // HiddenChunks has exactly 32 contiguous live slices. SAO's relocated
    // tail validated this regular Broadcast1 topology on all three seeds.
    // Unlike the earlier failing short CustomBroadcast, this needs no
    // routing bitmap and preserves the full 32-term sum and EPS.
    let ms: DmTensor<f32, Chip, C, m![1 # 8, Dummy32], m![1 # 8]> = ctx
        .main
        .begin(partial.view())
        .fetch::<m![1], m![1 # 8]>()
        .switch::<m![1 # 8, Dummy32], m![H / 120]>(SwitchConfig::Broadcast1 { slice1: 32, slice0: 1 })
    ...
    let ms: DmTensor<f32, Chip, C, HiddenChunks, m![1 # 8]> = unsafe { ms.reshape() };

    // Keep the DM boundary after the complete sum.
    // Only the final sqrt result bypasses the DM commit/reload pair.
    ctx.main
        .begin(ms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
...
pub(crate) fn normalize_chunk_rows(
    ...
    let rms_vrf = root_mean_square_fused(ctx, x);
