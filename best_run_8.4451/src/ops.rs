...
pub fn sliding_attention_output(
    ...
    let ctx = device;
    sliding::output::attention_output(ctx, x, post_attn_rms_weight, o_weight, o_weight_scale, residual_hbm);
}
...
pub fn decoder_feedforward(
    ...
    let ctx = device;
    let x = shared::hidden::normalize_replicated(ctx, residual_hbm, pre_ff_rms_weight);
    let down_scale = shared::hidden::broadcast_scalar_f32_chunks(ctx, down_global_scale);
    ...
    let x = shared::hidden::normalize_chunk_rows(ctx, &x, post_ff_rms_weight);
    let x = shared::hidden::add_residual_chunks(ctx, &x, residual_hbm);
    let gate = shared::hidden::scalar_bf16_vrf(ctx, layer_scalar);
    let x = shared::hidden::scale_chunks(ctx, &x, &gate);
    shared::hidden::store_chunk_rows(ctx, x, residual_hbm);
}
